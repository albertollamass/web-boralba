...
import { normalizeText, searchProducts } from '../../domain/search/search'
import Seo from '../components/Seo'
import { canonicalFor, breadcrumbJsonLd } from '../../domain/site/site'
import '../styles/catalog.css'

/* Primer nivel de navegación: todo lo que cuelga de la raíz en Firebase.
   Las categorías nuevas aparecen solas, sin tocar código. */
...
export default function Productos() {
  ...
  /* Familias: hijas directas de la raíz en Firebase, ordenadas por homeOrder
     (editable desde /admin) y luego por nombre. */
  const families = useMemo(() => (
    getChildren(ROOT.slug)
      .slice()
      .sort((a, b) => (a.homeOrder ?? 999) - (b.homeOrder ?? 999) || String(a.name || '').localeCompare(String(b.name || ''), 'es'))
  ), [getChildren, ROOT.slug])
  ...
  const liveCategories = useMemo(() => {
    ...
    if (!clean) return []
    return categories
      .filter((category) => category.slug !== ROOT.slug)
      .filter((category) => {
