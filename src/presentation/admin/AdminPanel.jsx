...
import { DAILY_READ_LIMIT, READ_ALERT_AT, useDailyReads } from '../../application/telemetry/usage'
...
import { generateSlug } from '../../domain/catalog/slug'
...
export default function AdminPanel({ telemetry, documentStorage }) {
  ...
  // Cuota diaria de la base de datos: muestra el consumo para anticiparse al corte.
  const dailyReads = useDailyReads(telemetry)
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 18 }}>
              <div>
                ...
                <span className="muted" style={{ fontSize: '0.88rem' }}>
                  ...
                  {dailyReads != null && (
                    <> · <span style={dailyReads >= READ_ALERT_AT ? { color: '#b00', fontWeight: 700 } : undefined}>
                      {dailyReads.toLocaleString('es')} / {DAILY_READ_LIMIT.toLocaleString('es')} lecturas hoy
                    </span></>
                  )}
                </span>
              </div>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                {cloudEnabled && (
                  <span
                    className={`sync-badge sync-${syncStatus === 'cache' ? 'cloud' : syncStatus}`}
                    title={
                      syncStatus === 'cloud'
                        ? 'Los productos se guardan en Firebase'
                        : syncStatus === 'cache'
                          ? 'Mostrando copia local (se actualiza sola cada 30 min)'
                          : syncStatus === 'error'
...
function CategoryForm({ initial, ctx, onCancel, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      alert('No se pudo guardar en la nube. Revisa tu conexión o los permisos de Firebase.')
    } finally {
      setSaving(false)
    }
  }
